...
\def\firstAuthorLast{JJ Merelo {et~al.}} %use et al only if is more than 1 author
\def\Authors{Juan-Julián Merelo-Guervós\,$^{1,2,*}$, P.A. Castillo\,$^{1,2}$ and Co-Author\,$^{1,2}$}
...
of sciences such as artificial life. In this short paper we will 
% of data-driven sciences?  because cloud is at the technological
% level? - Mario
% artificial life is not a science, rather a
% discipline. Computing-intensive sciences? Alife is not data driven - JJ
review of how the world of cloud computing has intersected the
% In this short review we will make a review of how ..
% we will [analyze|describe|center on|show] how the world of ..
% seems too repetitive review .. review - Mario
% Changed - JJ
artificial life field, and how it has been used as an
inspiration for new models or the design and implementation of new and powerful
...
to other, {\em classical}, methodologies. % those classical methodologies should be enumerated  [pedro]


\begin{itemize}

\item Infrastructure is fully automatized and described by software;
  % Maybe we could be more specific about the Infrastructure part?
  % the keyword here been provisioning. To differentiate from administration
  % and maintenance that is also
  % provided in traditional full-managed servers.
  ...
  % ... with hardware resources and hosted
  % services specified at the same time. - Mario
  % Please change at will - JJ
...
\item Applications are a loose collection of resources which interact
  ...
  % product owners -> service providers? - Mario
  % changed - JJ
  ...
  becoming active only after % they activate after they receive information
  % becoming active only after they receive information - Mario
  % changed - JJ
...
\cite{mericloud}
% \citep{merelo2011evostar} 
% We could change the current citation (the one that appears in the References section) by the paper by K. Meri et al  (http://dblp.uni-trier.de/rec/bibtex/journals/nc/MeriAMGCGL13) <= \cite{mericloud} in geneura.bib
